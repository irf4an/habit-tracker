...
export const StatsView: React.FC<StatsViewProps> = ({ habits, isDarkMode = true }) => {
  ...
  // Combined Multi-Habit Activity Heatmap: Adaptive (28 weeks on desktop, 18 on mobile)
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      {/* 2x2 METRIC GRID WITH STRONG TYPOGRAPHY HIERARCHY */}
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        {/* 1. Total Check-ins */}
        <div
          className={`border rounded-2xl p-4 sm:p-5 transition-all flex flex-col justify-between ${
            isDarkMode
        ...
        >
          <div>
            ...
            <div className={`text-3xl sm:text-4xl font-extrabold tracking-tight font-sans ${isDarkMode ? 'text-white' : 'text-zinc-950'}`}>
              {totalCompletions}
            </div>
          </div>
          <p className={`text-[11px] sm:text-xs mt-2 ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            All-time lifetime logs
          </p>
        </div>
        ...
        >
          <div>
            ...
            <div className={`text-3xl sm:text-4xl font-extrabold tracking-tight font-sans flex items-baseline gap-1.5 ${isDarkMode ? 'text-white' : 'text-zinc-950'}`}>
              {bestOverallStreak}
        ...
        >
          ...
          <p className={`text-[11px] sm:text-xs mt-2 ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Routines being tracked
          </p>
        </div>
      </div>

      {/* COMBINED MULTI-HABIT HEATMAP MATRIX */}
      <div
      ...
      >
        <div className="flex items-center justify-between gap-3 mb-4">
          <h3 className={`text-sm sm:text-base font-bold flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
            ...
            Unified Daily Routine Matrix
          </h3>
          <span className={`text-[11px] font-mono ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Past 30 Weeks
          </span>
        </div>
        ...
        <div className="overflow-x-auto pb-1 no-scrollbar touch-pan-x flex justify-start sm:justify-center">
          <div className="min-w-fit flex flex-col gap-1.5 select-none">
            {/* Months Header */}
            <div className={`flex text-[10px] font-mono pl-6 mb-1 relative h-4 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
            ...
            <div className="flex gap-1.5 items-start">
              {/* Day Labels */}
              <div className={`flex flex-col gap-[3px] text-[9px] font-mono pr-1.5 pt-[1px] ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
                {dayLabels.map((lbl, dIdx) => (
                  <div key={dIdx} className="h-3.5 leading-[14px] w-5 text-right">
                    {dIdx % 2 === 0 ? lbl : ''}
                  </div>
                ))}
              </div>
                  ...
                  <div key={wIdx} className="flex flex-col gap-[3px]">
                    {week.map((day) => {
                      ...
                      return (
                        <div
                          key={day.dateStr}
                          title={`${day.dateStr}: ${doneCount} of ${habits.length} habits completed`}
                          className={`w-3.5 h-3.5 rounded-[2.5px] transition-transform duration-100 ${
            ...
            {/* Matrix Legend */}
            <div className="flex items-center justify-end gap-1.5 text-[10px] font-mono text-zinc-500 mt-3 pr-1">
              <span>0</span>
              <div className={`w-3 h-3 rounded-[2px] ${isDarkMode ? 'bg-[#181824]' : 'bg-[#e4e4e7]'}`} />
              <div className="w-3 h-3 rounded-[2px] bg-[#ddd6fe]" />
              <div className="w-3 h-3 rounded-[2px] bg-[#b5838d]" />
              <div className="w-3 h-3 rounded-[2px] bg-[#9b5de5]" />
              <div className="w-3 h-3 rounded-[2px] bg-[#8338ec]" />
              <span>100% All Done</span>
            </div>
          </div>
        </div>
      </div>

      {/* MOST CONSISTENT DAYS (WEEKDAY PERFORMANCE) */}
      <div className={`border rounded-2xl p-5 sm:p-6 shadow-xl ${isDarkMode ? 'bg-[#121217] border-[#8338ec]/30' : 'bg-white border-zinc-200'}`}>
        ...
        <div className="grid grid-cols-7 gap-2 sm:gap-4 text-center font-mono pt-2">
          {dayOfWeekPerformance.map((day) => (
            <div key={day.name} className="flex flex-col items-center gap-2 group">
              ...
              {/* Taller Column Bar for Desktop (h-28 to h-40) */}
              <div className={`w-full h-28 sm:h-36 rounded-2xl flex items-end justify-center p-1.5 transition-all ${
              ...
              <div className="flex flex-col items-center">
                ...
                <span className={`text-[10px] mt-0.5 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>{day.count} logs</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* CLEAN & UNIFIED HABIT PERFORMANCE RANKING (REPLACES CLUTTERED LEADERBOARD + BREAKDOWN) */}
      <div className={`border rounded-2xl p-4 sm:p-5 shadow-xl ${isDarkMode ? 'bg-[#121217] border-[#8338ec]/30' : 'bg-white border-zinc-200'}`}>
        <div className="flex items-center justify-between mb-3">
          <h3 className={`text-sm font-bold flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
            <Target className="w-4 h-4 text-[#8338ec]" />
            Habit Consistency Rankings
          </h3>
          <span className={`text-[10px] font-mono ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Sorted by Streak
          </span>
        </div>
              ...
              >
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <h4 className={`font-bold text-xs sm:text-sm truncate ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
                        {habit.name}
                      </h4>
                      {habit.category && (
                    ...
                    <div className={`text-[10px] font-mono ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
                      {totalCompleted} check-ins · Best: {bestStreak}d
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 shrink-0">
                  ...
                  <div className="hidden sm:flex flex-col items-end w-20">
                    <span className={`text-[9.5px] font-mono font-medium ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
                      {completionRate}% (30d)
                    </span>
                  ...
                  {/* Streak Badge */}
                  <div
                    className="px-2.5 py-1 rounded-lg text-[10.5px] font-mono font-bold flex items-center gap-1 shrink-0"
                    style={{
